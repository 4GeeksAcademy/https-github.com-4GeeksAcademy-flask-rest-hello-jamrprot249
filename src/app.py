...
def get_user_favorites():
    user_id = 1 # Simulamos usuario 1 como dice el README
...
def get_person(people_id):
    ...
    if not person: return jsonify({"msg": "No existe"}), 404
...
def get_planet(planet_id):
    ...
    if not planet: return jsonify({"msg": "No existe"}), 404
...
def add_fav_planet(planet_id):
    ...
    db.session.commit()
    return jsonify({"msg": "Planeta favorito añadido"}), 200
...
def add_fav_person(people_id):
    ...
    db.session.add(new_fav)
    db.session.commit()
    return jsonify({"msg": "Personaje favorito añadido"}), 200
